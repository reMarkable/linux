...
void mxc_isi_cap_frame_write_done(struct mxc_isi_dev *mxc_isi)
{
	...
	list_move_tail(mxc_isi->isi_cap.out_pending.next,
		       &mxc_isi->isi_cap.out_active);
}
...
static int cap_vb2_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	for (i = 0; i < mxc_isi->pix.num_planes; i++) {
		...
		mxc_isi->discard_buffer[i] =
			dma_alloc_coherent(&mxc_isi->pdev->dev,
	...
	list_add_tail(&mxc_isi->buf_discard[0].list,
		      &mxc_isi->isi_cap.out_discard);

	mxc_isi->buf_discard[1].discard = true;
	list_add_tail(&mxc_isi->buf_discard[1].list,
		      &mxc_isi->isi_cap.out_discard);
...
static void cap_vb2_stop_streaming(struct vb2_queue *q)
{
	...
	while (!list_empty(&mxc_isi->isi_cap.out_active)) {
		buf = list_entry(mxc_isi->isi_cap.out_active.next,
				 struct mxc_isi_buffer, list);
		list_del(&buf->list);
		if (buf->discard)
			continue;
	...
	while (!list_empty(&mxc_isi->isi_cap.out_pending)) {
		buf = list_entry(mxc_isi->isi_cap.out_pending.next,
				 struct mxc_isi_buffer, list);
		list_del(&buf->list);
		vb2_buffer_done(&buf->v4l2_buf.vb2_buf, VB2_BUF_STATE_ERROR);
	}
	...
	while (!list_empty(&mxc_isi->isi_cap.out_discard)) {
		buf = list_entry(mxc_isi->isi_cap.out_discard.next,
				 struct mxc_isi_buffer, list);
		list_del(&buf->list);
	}

	list_for_each_entry_safe(buf, tmp, &mxc_isi->isi_cap.out_active, list) {
		list_del(&buf->list);
		vb2_buffer_done(&buf->v4l2_buf.vb2_buf, VB2_BUF_STATE_ERROR);
	}

	list_for_each_entry_safe(buf, tmp, &mxc_isi->isi_cap.out_pending, list) {
		list_del(&buf->list);
		vb2_buffer_done(&buf->v4l2_buf.vb2_buf, VB2_BUF_STATE_ERROR);
	}
 ...
 */
static inline struct mxc_isi_dev *ctrl_to_mxc_isi(struct v4l2_ctrl *ctrl)
{
	return container_of(ctrl->handler, struct mxc_isi_dev, ctrls.handler);
}
...
static struct v4l2_subdev *mxc_get_remote_subdev(struct mxc_isi_dev *mxc_isi,
...
static int mxc_isi_capture_open(struct file *file)
{
	...
	dev_dbg(&mxc_isi->pdev->dev, "ISI(%d)\n", mxc_isi->id);

	if (mxc_isi->is_m2m) {
		v4l2_err(&mxc_isi->isi_cap.sd,
			 "ISI channel[%d] is busy\n", mxc_isi->id);
		return -EBUSY;
	}
	atomic_inc(&mxc_isi->open_count);
	mxc_isi->is_m2m = 0;

	sd = mxc_get_remote_subdev(mxc_isi, __func__);
	if (!sd)
		goto fail;
	...
	if (ret) {
		v4l2_err(&mxc_isi->isi_cap.sd, "Call subdev s_power fail!\n");
		pm_runtime_put(dev);
		goto fail;
	}
...
static int mxc_isi_capture_release(struct file *file)
{
	...
	if (ret) {
		v4l2_err(&mxc_isi->isi_cap.sd, "%s fail\n", __func__);
		mutex_unlock(&mxc_isi->lock);
		goto label;
	}
	mutex_unlock(&mxc_isi->lock);
	...
	if (ret < 0 && ret != -ENOIOCTLCMD) {
		v4l2_err(&mxc_isi->isi_cap.sd, "%s s_power fail\n", __func__);
		goto label;
	}
				      ...
				      struct v4l2_format *f)
{
	...
	if (i >= ARRAY_SIZE(mxc_isi_out_formats)) {
		v4l2_err(&mxc_isi->isi_cap.sd, "format(%.4s) is not support!\n",
			 (char *)&pix->pixelformat);
		return -EINVAL;
	}

	if (pix->width <= 0 || pix->height <= 0) {
		v4l2_err(&mxc_isi->isi_cap.sd, "%s, W/H=(%d, %d) is not valid\n"
...
static int mxc_isi_source_fmt_init(struct mxc_isi_dev *mxc_isi)
{
	...
	src_sd = mxc_get_remote_subdev(mxc_isi, __func__);
	if (!src_sd)
		return -EINVAL;
	...
	if (ret < 0 && ret != -ENOIOCTLCMD) {
		v4l2_err(&mxc_isi->isi_cap.sd, "set remote fmt fail!\n");
		return -EINVAL;
	}
	...
	if (ret < 0 && ret != -ENOIOCTLCMD) {
		v4l2_err(&mxc_isi->isi_cap.sd, "get remote fmt fail!\n");
		return -EINVAL;
	}
			      ...
			      const struct media_pad *remote, u32 flags)
{
	...
	if (local->flags & MEDIA_PAD_FL_SOURCE) {
		...
		case MXC_ISI_SD_PAD_SOURCE_MEM:
			break;
		default:
			dev_err(&mxc_isi->pdev->dev, "invalid source pad\n");
			return -EINVAL;
		}
