...
static void dcss_dpr_write(struct dcss_dpr *dpr, int ch_num,
			   u32 val, u32 ofs)
{
	dcss_ctxld_write(dpr->ctxld, dpr->ctx_id,
...
int dcss_dpr_init(struct dcss_dev *dcss, unsigned long dpr_base)
{
	...
	return dcss_dpr_ch_init_all(dpr, dpr_base);
}
...
void dcss_dpr_set_res(struct dcss_dpr *dpr, int ch_num, u32 xres, u32 yres)
{
	...
	u32 pix_format = dpr->ch[ch_num].format.format;
		       ...
		       u32 chroma_base_addr, u16 pitch)
{
	...
	dcss_dpr_write(dpr, ch_num, luma_base_addr,
		       DCSS_DPR_FRAME_1P_BASE_ADDR);

	dcss_dpr_write(dpr, ch_num, chroma_base_addr,
...
static void dcss_dpr_argb_comp_sel(struct dcss_dpr *dpr, int ch_num,
...
static void dcss_dpr_pix_size_set(struct dcss_dpr *dpr, int ch_num,
...
static void dcss_dpr_uv_swap(struct dcss_dpr *dpr, int ch_num, bool swap)
{
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];
...
static void dcss_dpr_y_uv_swap(struct dcss_dpr *dpr, int ch_num, bool swap)
{
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];
...
static void dcss_dpr_2plane_en(struct dcss_dpr *dpr, int ch_num, bool en)
{
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];
...
static void dcss_dpr_yuv_en(struct dcss_dpr *dpr, int ch_num, bool en)
{
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];
...
static void dcss_dpr_tile_set(struct dcss_dpr *dpr, int ch_num,
			      enum dcss_tile_type tile)
{
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];
...
static void dcss_dpr_rtram_set(struct dcss_dpr *dpr, int ch_num,
			       u32 pix_format)
{
	u32 val, mask;
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];
...
static void dcss_dpr_setup_components(struct dcss_dpr *dpr, int ch_num,
				      const struct drm_format_info *format)
{
	...
	dcss_dpr_uv_swap(dpr, ch_num, uv_swap);

	dcss_dpr_y_uv_swap(dpr, ch_num, y_uv_swap);

	if (!format->is_yuv) {
		if (!to_comp_sel(format->format, &a_sel, &r_sel,
				 &g_sel, &b_sel)) {
			dcss_dpr_argb_comp_sel(dpr, ch_num, a_sel, r_sel,
					       g_sel, b_sel);
		} else {
			dcss_dpr_argb_comp_sel(dpr, ch_num, 3, 2, 1, 0);
		}
	} else {
		dcss_dpr_argb_comp_sel(dpr, ch_num, 0, 0, 0, 0);
	}
}

void dcss_dpr_tile_derive(struct dcss_dpr *dpr,
			  int ch_num,
			  uint64_t modifier)
{
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];
	...
	case 0:
		...
		case DRM_FORMAT_MOD_LINEAR:
			dcss_dpr_tile_set(dpr, ch_num, TILE_LINEAR);
			ch->tile = TILE_LINEAR;
			break;
		case DRM_FORMAT_MOD_VIVANTE_TILED:
			dcss_dpr_tile_set(dpr, ch_num, TILE_GPU_STANDARD);
			ch->tile = TILE_GPU_STANDARD;
			break;
		case DRM_FORMAT_MOD_VIVANTE_SUPER_TILED:
			dcss_dpr_tile_set(dpr, ch_num, TILE_GPU_SUPER);
			 ...
			 bool modifiers_present)
{
	...
	dev_dbg(dpr->dev, "pix_format = %s, colorspace = %s, bpp = %d\n",
		drm_get_format_name(format->format, &format_name),
		format->is_yuv ? "YUV" : "RGB", format->depth);

	dcss_dpr_yuv_en(dpr, ch_num, format->is_yuv);

	dcss_dpr_pix_size_set(dpr, ch_num, format);

	dcss_dpr_setup_components(dpr, ch_num, format);

	dcss_dpr_2plane_en(dpr, ch_num, format->num_planes == 2 ? true : false);

	dcss_dpr_rtram_set(dpr, ch_num, format->format);
}
