...
int dcss_dtg_init(struct dcss_dev *dcss, unsigned long dtg_base)
{
	...
	return dcss_dtg_irq_config(dtg, to_platform_device(dcss->dev));
}
...
void dcss_dtg_sync_set(struct dcss_dtg *dtg, struct videomode *vm)
{
	...
	dev_dbg(dtg->dev, "hfront_porch = %d\n", vm->hfront_porch);
	dev_dbg(dtg->dev, "hback_porch = %d\n", vm->hback_porch);
	dev_dbg(dtg->dev, "hsync_len = %d\n", vm->hsync_len);
	dev_dbg(dtg->dev, "hactive = %d\n", vm->hactive);
	dev_dbg(dtg->dev, "vfront_porch = %d\n", vm->vfront_porch);
	dev_dbg(dtg->dev, "vback_porch = %d\n", vm->vback_porch);
	dev_dbg(dtg->dev, "vsync_len = %d\n", vm->vsync_len);
	dev_dbg(dtg->dev, "vactive = %d\n", vm->vactive);
	dev_dbg(dtg->dev, "pixelclock = %lu\n", vm->pixelclock);
	...
	if (dtg->hdmi_output) {
		...
		if (err < 0)
			dev_warn(dtg->dev, "clk_set_parent() returned %d",
				 err);
