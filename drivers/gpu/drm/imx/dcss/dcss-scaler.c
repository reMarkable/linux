...
static void dcss_scaler_write(struct dcss_scaler *scl, int ch_num,
			      u32 val, u32 ofs)
{
	dcss_ctxld_write(scl->ctxld, scl->ctx_id,
...
int dcss_scaler_init(struct dcss_dev *dcss, unsigned long scaler_base)
{
	...
	return dcss_scaler_ch_init_all(scaler, scaler_base);
}
...
void dcss_scaler_ch_enable(struct dcss_scaler *scl, int ch_num, bool en)
{
	...
	if (en)
		dcss_scaler_write(scl, ch_num, ch->sdata_ctrl,
...
static void dcss_scaler_yuv_enable(struct dcss_scaler *scl, int ch_num, bool en)
{
	struct dcss_scaler_ch *ch = &scl->ch[ch_num];
...
static void dcss_scaler_rtr_8lines_enable(struct dcss_scaler *scl, int ch_num,
					  bool en)
{
	struct dcss_scaler_ch *ch = &scl->ch[ch_num];
...
static void dcss_scaler_bit_depth_set(struct dcss_scaler *scl, int ch_num,
				      int depth)
{
	u32 val;
...
static void dcss_scaler_format_set(struct dcss_scaler *scl, int ch_num,
				   enum buffer_format src_fmt,
				   enum buffer_format dst_fmt)
{
	dcss_scaler_write(scl, ch_num, src_fmt, DCSS_SCALER_SRC_FORMAT);
	dcss_scaler_write(scl, ch_num, dst_fmt, DCSS_SCALER_DST_FORMAT);
}

static void dcss_scaler_res_set(struct dcss_scaler *scl, int ch_num,
...
static void dcss_scaler_fractions_set(struct dcss_scaler *scl, int ch_num,
				      int src_xres, int src_yres,
				      int dst_xres, int dst_yres,
				      ...
				      enum chroma_location src_chroma_loc)
{
	...
	dcss_scaler_write(scl, ch_num, 0, DCSS_SCALER_V_LUM_START);
	dcss_scaler_write(scl, ch_num, l_vinc, DCSS_SCALER_V_LUM_INC);

	dcss_scaler_write(scl, ch_num, 0, DCSS_SCALER_H_LUM_START);
	dcss_scaler_write(scl, ch_num, l_hinc, DCSS_SCALER_H_LUM_INC);

	dcss_scaler_write(scl, ch_num, c_vstart, DCSS_SCALER_V_CHR_START);
	dcss_scaler_write(scl, ch_num, c_vinc, DCSS_SCALER_V_CHR_INC);

	dcss_scaler_write(scl, ch_num, c_hstart, DCSS_SCALER_H_CHR_START);
	dcss_scaler_write(scl, ch_num, c_hinc, DCSS_SCALER_H_CHR_INC);
}
...
static void dcss_scaler_program_5_coef_set(struct dcss_scaler *scl, int ch_num,
					   int base_addr,
					   int coef[][PSC_NUM_TAPS])
{
	int i, phase;
...
static void dcss_scaler_program_7_coef_set(struct dcss_scaler *scl, int ch_num,
					   int base_addr,
					   int coef[][PSC_NUM_TAPS])
{
	int i, phase;
...
static void dcss_scaler_yuv_coef_set(struct dcss_scaler *scl, int ch_num,
				     ...
				     int dst_yres)
{
	...
	dcss_scaler_program_7_coef_set(scl, ch_num,
	...
	if (program_5_taps)
		dcss_scaler_program_5_coef_set(scl, ch_num,
...
static void dcss_scaler_rgb_coef_set(struct dcss_scaler *scl, int ch_num,
				     int src_xres, int src_yres, int dst_xres,
				     int dst_yres)
{
	int coef[PSC_STORED_PHASES][PSC_NUM_TAPS];
...
static void dcss_scaler_set_rgb10_order(struct dcss_scaler *scl, int ch_num,
		       ...
		       u32 vrefresh_hz)
{
	...
	if (format->is_yuv) {
		dcss_scaler_yuv_enable(scl, ch_num, true);
		...
		dcss_scaler_yuv_enable(scl, ch_num, false);

		pixel_depth = format->depth;
	}

	dcss_scaler_fractions_set(scl, ch_num, src_xres, src_yres, dst_xres,
				  dst_yres, src_format, dst_format,
				  PSC_LOC_HORZ_0_VERT_1_OVER_4);

	if (format->is_yuv)
		dcss_scaler_yuv_coef_set(scl, ch_num, src_format, dst_format,
					 use_5_taps, src_xres, src_yres,
					 dst_xres, dst_yres);
	else
		dcss_scaler_rgb_coef_set(scl, ch_num, src_xres, src_yres,
					 dst_xres, dst_yres);

	dcss_scaler_rtr_8lines_enable(scl, ch_num, rtr_8line_en);
	dcss_scaler_bit_depth_set(scl, ch_num, pixel_depth);
	dcss_scaler_set_rgb10_order(scl, ch_num, format);
	dcss_scaler_format_set(scl, ch_num, src_format, dst_format);
	dcss_scaler_res_set(scl, ch_num, src_xres, src_yres,
