	...
	if (cur_if->ifmode == ISTA_MODE) {
		if (event_type == WLC_E_LINK) {
			if (!(flags & WLC_EVENT_MSG_LINK)) {
				WL_MSG(cur_if->ifname, "[%c] Link Down with %pM, reason=%d\n",
					cur_if->prefix, &e->addr, reason);
				wl_clr_isam_status(cur_if, STA_CONNECTED);
			} else {
			...
			WL_MSG(cur_if->ifname,
				"connect failed event=%d e->status %d e->reason %d\n",
		...
		if ((event_type == WLC_E_SET_SSID && status == WLC_E_STATUS_SUCCESS) ||
			...
			WL_MSG(cur_if->ifname, "[%c] disassociated device %pM, reason=%d\n",
				cur_if->prefix, &e->addr, reason);
		} else if (event_type == WLC_E_DEAUTH_IND ||
				(event_type == WLC_E_DEAUTH && reason != DOT11_RC_RESERVED)) {
			WL_MSG(cur_if->ifname, "[%c] deauthenticated device %pM, reason=%d\n",
				cur_if->prefix, &e->addr, reason);
		}
	}
