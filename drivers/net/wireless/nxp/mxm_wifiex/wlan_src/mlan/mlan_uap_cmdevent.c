 ...
 */
mlan_status
wlan_cmd_set_get_band_steering_cfg(IN pmlan_private pmpriv,
				   ...
				   IN t_u16 cmd_action, IN t_void *pdata_buf)
{
	...
	cmd->size = wlan_cpu_to_le16(sizeof(HostCmd_DS_BAND_STEERING) +
				     S_DS_GEN);
		      ...
		      IN mlan_ioctl_req *pioctl_buf)
{
	...
	if (pioctl_buf) {
		...
		buf = (t_u8 *)sta_list + sizeof(HostCmd_DS_STA_LIST);
			 ...
			 IN t_void *pdata_buf, IN t_void *pcmd_buf)
{
	...
	case HostCmd_CMD_RATE_ADAPT_CFG:
		ret = wlan_cmd_rate_adapt_cfg(pmpriv, cmd_ptr,
					      cmd_action, pdata_buf);
		break;
	case HostCmd_CMD_CCK_DESENSE_CFG:
		ret = wlan_cmd_cck_desense_cfg(pmpriv, cmd_ptr,
					       cmd_action, pdata_buf);
		break;
 ...
 */
mlan_status
wlan_uap_set_uap_max_sta(IN pmlan_private pmpriv, IN t_u8 uap_max_sta)
{
